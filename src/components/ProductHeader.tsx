...
export function ProductHeader({ product, size = 'medium', additionalBadges = [] }: ProductHeaderProps) {
    const isLarge = size === 'large'
    const headerClass = isLarge ? 'text-2xl font-bold' : 'text-xl font-bold'
    const containerClass = isLarge ? 'px-8 py-6' : 'px-6 py-4'

    return (
        <div className={`bg-gradient-to-r from-slate-50 to-blue-50 ${containerClass} border-b border-slate-200`}>
            <div className="flex justify-between items-start">
                <div className={isLarge ? "flex-1" : ""}>
                    {product.product_name && (
                        <h2 className={`${headerClass} text-slate-900 mb-2 ${isLarge ? 'leading-tight' : ''}`}>
                            {product.product_name}
                        </h2>
                    )}
