...
export default function CategoriesPage() {
    ...
    async function searchCategories(query: string): Promise<Category[]> {
        const { data } = await supabase
            ...
            .or(`header.ilike.%${query}%,id.eq.${isNaN(Number(query)) ? 0 : Number(query)}`)
            .limit(10)
        return data || []
    }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
            <Header title="Список категорий" subtitle="Просмотр категорий с готовыми описаниями" />
            ...
            <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
                ...
                {/* Пагинация сверху */}
                {!loading && !selectedCategory && <PaginationBar page={page} total={total} pageSize={pageSize} onPageChange={setPage} remainingToConfirm={remainingToConfirm} />}
                                    ...
                                    {category.description ? (
                                        <div className="space-y-3">
                                            ...
                                            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                                                <SafeHtml html={category.description} className="rich-html rich-html-detailed" />
                                            </div>
                                        </div>
                                    ) : (
